...
BuildOrderQueue::BuildOrderQueue(Zeratul & bot)
    : m_bot(bot)
    , m_highestPriority(0)
    , m_lowestPriority(0)
...
void BuildOrderQueue::queueAsHighestPriority(const MacroAct & type, bool blocking)
{
...
void BuildOrderQueue::queueAsLowestPriority(const MacroAct & type, bool blocking)
{
    // the new priority will be higher
...
std::string BuildOrderQueue::getQueueInformation() const
{
    ...
    {
        const MacroAct & type = m_queue[m_queue.size() - 1 - i].type;
        ss << type.getName() << "\n";
    }
