...
MacroAct::MacroAct(const std::string & name, Zeratul & bot)
	: _type(MacroActs::Default)
	, _macroLocation(MacroLocation::Anywhere)
{
...
const sc2::Point2D MacroAct::getMacroLocation(Zeratul & bot) const
{
...
int MacroAct::supplyRequired(Zeratul & bot) const
{
	if (isUnit())
	{
...
int MacroAct::mineralPrice(Zeratul & bot) const
{
	return isUnit() ?
...
int MacroAct::gasPrice(Zeratul & bot) const
{
	if (isCommand()) {
		return 0;
	}
...
const std::vector<sc2::UnitTypeID> & MacroAct::whatBuilds(Zeratul & bot) const
{
	if (isCommand()) {
